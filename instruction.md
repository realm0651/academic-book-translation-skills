...
PDF-I：只有图片、无可用文字层或文字层不可用；允许采用分阶段原文重建，不做一次性整本OCR。初始化时先依据页面图像/目录划分逻辑Parts；随后逐Part OCR当前页码范围，把OCR视为待校正转录，逐页核对标题、段落、专名、引文、数字、脚注、断词、错序等，生成`<BOOK_STEM>_partN_原文校正版.md`。原则上先完成全部计划Part的原文重建，再进入中译；原始PDF页面始终优先于OCR。无法可靠恢复的文字标待核对，不猜测。
初始化至少完成：判断PDF类型；浏览全书并定位目录、序言/前言、Introduction、正文、附录、Notes、References、Bibliography、Index等；判断Notes类型；规划逻辑Parts并记录精确物理页码、输出、脚注前缀和状态；记录BOOK_STEM、PDF/Notes类型、翻译范围、profile/特殊决策、Current state、Next action；建立初始glossary并检查状态。PDF-T/PDF-O的Next action通常为翻译Part1；PDF-I为原文重建Part1。之后只需“继续”。
...
# 四、文件连续使用与跨session恢复
当前环境能访问既有文件就继续用，不因换轮次/session要求重传。普通逐Part翻译最小恢复集合是原始PDF+最新版book_plan.md+最新版glossary.md+适用profile；PDF-I在翻译阶段还应使用当前Part已完成的`原文校正版.md`，不得重复OCR。已完成中译Part和旧assets不是下一Part常规前置条件。旧Part PDF不存在但原始PDF在时，按book_plan页码读取；不要要求上传全部历史Part。只有当前正文明确回指前文且glossary/book_plan不足时才按需读取旧译文。
...
# 十、PDF类型规则边界与优先级
PDF-T/PDF-O/PDF-I只改变“如何可靠读取原文”和PDF-I多出的原文重建checkpoint，不改变Notes/脚注规则、Part划分与book_plan页码定位、标题/表格/图片/Markdown规则、glossary滚动更新、最终整合及出版结构。
